 ...
 * PROPOSITION
 *
 * les lignes 67 et 70 sont à tester
 * il est possible également d'automatiser la collecte du nom des groupes
 * et de se passer de les déclarer dans le tableau _grpArray
 **************************************************************************** */

// Déclaration du nom des groupes
_grpArray = ["grp1", "grp2", "grp3", "grp4", "grp5"];

// pour chaque groupe possible 
for "_i" from 0 to ((count _grpArray) - 1) do {

	// le groupe représenté par la variable existe t'il ?
	if !(isNil (_grpArray select _i)) then {

		// transformation de la variable en groupe
		_grp 				= call compile format ["%1", _x];

		// séléction d'un point de spawn
		_j 					= floor (random (count _spawnHunter));
		_pos_mk_spawnHunter = getMarkerPos (_spawnHunter select _j);
		_spawnHunter 		= _spawnHunter - [_spawnHunter select _j];
